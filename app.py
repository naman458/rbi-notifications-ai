...
# RAG Setup (OpenAI - reliable and free tier)
embeddings = OpenAIEmbeddings(model="text-embedding-3-small")  # Free tier embedding
llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.1, api_key=os.getenv('OPENAI_API_KEY'))  # Use OPENAI_API_KEY

@st.cache_data(ttl=7200)
def load_docs():
    ist = ZoneInfo("Asia/Kolkata")
    two_weeks_ago = datetime.now(ist).date() - timedelta(days=14)
...
with tab2:
    vectorstore, recent_docs = load_docs()
